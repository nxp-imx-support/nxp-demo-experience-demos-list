...
count = 0
cap = cv2.VideoCapture("udpsrc port=5000 !application/x-rtp, encoding-name=JPEG, payload=26 ! rtpjpegdepay ! jpegparse ! jpegdec ! decodebin ! videoconvert ! appsink", cv2.CAP_GSTREAMER)
while(cap.isOpened()):
    ret, frame = cap.read()
    if ret:
