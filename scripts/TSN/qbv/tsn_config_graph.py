...
class ButtonWindow(Gtk.Window):
    def __init__(self):
        ...
        for item in ["No qbv (No Prioritization)", "Qbv1 (Video Prioritization)", "Qbv2 (Iperf Prioritization)"]:
...
def animate(i,x_axis:list,y1_axis:list,y2_axis:list):
    try: 
         ...
         ax1.clear()
         ax2.clear()
         ax1.plot(x_axis, y1_axis,'go-', linewidth=1, markersize=3, label = 'Iperf(Mbps)=%.0f'%(iperf))
         plt.setp(ax1.get_xticklabels(), rotation=30, horizontalalignment='right')
         ax2.plot(x_axis, y2_axis,'bo-', linewidth=1, markersize=3, label = 'FPS=%.2f'%(FPS))
