...
class DownloadGUI(Gtk.Window):
    ...
    def preload(self):
        ...
        self.cpu_model = utils.download_file("mobilenet_v1_1.0_224_quant.tflite")
        if self.cpu_model == -1 or self.cpu_model == -2 or self.cpu_model == -3:
            GLib.idle_add(self.status_label.set_text, "\n\nCannot download CPU Model!")
            return
        GLib.idle_add(self.status_label.set_text, "\n\nDownloading NPU model...")
        self.npu_model = utils.download_file("mobilenet_v1_1.0_224_quant_vela.tflite")
        if self.npu_model == -1 or self.npu_model == -2 or self.npu_model == -3:
...
def main():
    ...
    window = DownloadGUI()
    window.show_all()
    download_thread = threading.Thread(target=window.preload)
