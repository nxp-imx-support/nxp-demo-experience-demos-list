...
class FaceDemo:
    ...
    def setup_inferences(self, backend):
        ...
        self.face_models.append(
            self.setup_model("ssd_mobilenet_v2_face_quant_postprocess.tflite", backend)
        )
        self.face_models.append(
            self.setup_model("facenet_int_quantized.tflite", backend)
        )
    ...
    def setup_model(self, name, backend):
        ...
        if backend == "NPU":
            ...
            interpreter = tflite.Interpreter(
                model_path=path, num_threads=4, experimental_delegates=[ext_delegate]
            )
        else:
        ...
        dummy_data = np.zeros(
            (1, input_size_w, input_size_h, 3), dtype=input_info[0]["dtype"]
        )
        interpreter.set_tensor(input_info[0]["index"], dummy_data)
        interpreter.invoke()
        return Model(
            path, interpreter, input_size_w, input_size_h, input_info, output_info
        )
    ...
    def process_frame(self, frame):
        ...
        if self.mode == 1:
            ...
            if (time.perf_counter() - self.countdown) < COUNTDOWN_TIME:
                time_left = round(
                    COUNTDOWN_TIME - (time.perf_counter() - self.countdown), 2
                )
                cv2.putText(
                    frame,
                    "Taking picture in " + str(time_left) + " seconds...",
                    (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1,
                    (255, 0, 0),
        ...
        for face in faces:
            ...
            if self.mode == 0 or self.mode == 1:
                ...
                cv2.rectangle(
                    frame,
                    ((int(face[1] * self.width)), (int(face[0] * self.height))),
                    ((int(face[3] * self.width)), (int(face[2] * self.height))),
                    color,
                    2,
                )
                cv2.putText(
                    frame,
                    face_info[0] + " " + str(face_info[1]) + "%",
                    ((int(face[1] * self.width)), (int(face[0] * self.height - 10))),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0,
                    color,
                    4,
            ...
            if self.mode == 2:
                ...
                if GUI:
                    GLib.idle_add(cv2.imshow, "i.MX Face Recognition Demo", blank_frame)
        ...
        cv2.putText(
            frame,
            "Detection time: " + detect_time[0] + " IPS (" + detect_time[1] + " ms)",
            (5, self.height - 35),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.75,
            (255, 255, 255),
    ...
    def find_faces(self, frame):
        ...
        face_boxes = self.face_models[0].interpreter.get_tensor(
            self.face_models[0].output_info[0]["index"]
        )[0]
        face_scores = self.face_models[0].interpreter.get_tensor(
            self.face_models[0].output_info[2]["index"]
        )[0]
        face_total = self.face_models[0].interpreter.get_tensor(
            self.face_models[0].output_info[3]["index"]
        )[0]
        face_output = []
        for face in range(int(face_total)):
            if face_scores[face] > DEFAULT_DETECTION_ACCURACY and self.valid_face(
                face_boxes[face]
            ):
    ...
    def id_face(self, face):
        ...
        face_map = self.face_models[1].interpreter.get_tensor(
            self.face_models[1].output_info[0]["index"]
        )[0]
    ...
    def run_inference(self, frame, model):
        ...
        input_img = cv2.resize(frame, [model.input_size_w, model.input_size_h])
...
class MainWindow(Gtk.Window):
    ...
    def on_change_start(self, widget):
        ...
        cam_thread = threading.Thread(
            ...
            args=(
                FACE_DEMO,
                self.backend_select.get_active_text(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gui", type=int, default=1, help="Run with GUI")
    parser.add_argument("--show", type=int, default=1, help="Show output")
    parser.add_argument("--npu", type=int, default=1, help="Use NPU")
