...
class ISPDemo(Gtk.Window):
    ...
    def __init__(self):
        ...
        # BLS Layout
        bls_grid = Gtk.Grid(row_homogeneous=True, column_spacing=15, row_spacing=15)
    ...
    def on_change_filter_set(self, unused):
        ...
        self.change_isp(
            "<id>:<filter.s.cfg>;<auto>:false;<denoise>:"
            + str(int(den))
            + ";<sharpen>:"
            + str(int(sharp))
        )
    ...
    def change_isp(self, command):
        ...
        os.system("v4l2-ctl -d " + CAM + " -c viv_ext_ctrl='{" + command + "}'")
        ...
        if dev.get_display_name() == "VIV":
            ...
            for i in range(dev_caps.get_size()):
                ...
                if FRAMERATE != "0/0":
                    ...
                    break
        if CAM is not None:
            Popen(["gst-launch-1.0", "v4l2src", "device=" + CAM, "!", "waylandsink"])
            break
    time.sleep(2)
    window = ISPDemo()
